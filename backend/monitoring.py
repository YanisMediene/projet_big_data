...
class MetricsCollector:
    ...
    def __init__(self):
        self.metrics = {
            "predictions": {"total": 0, "success": 0, "errors": 0, "latency_ms": []},
            "corrections": {"total": 0, "by_category": {}},
            "games": {"created": 0, "completed": 0, "active": 0},
            "retraining": {"triggered": 0, "success": 0, "failures": 0},
        }

    def record_prediction(
        self, success: bool, latency_ms: float, category: Optional[str] = None
    ):
        ...
        if len(self.metrics["predictions"]["latency_ms"]) > 1000:
            self.metrics["predictions"]["latency_ms"] = self.metrics["predictions"][
                "latency_ms"
            ][-1000:]
    ...
    def log_metrics(self):
        ...
        logger.info(
            f"Predictions: {metrics['predictions']['total']} total, "
        ...
        if "latency_p95" in metrics["predictions"]:
            logger.info(
                f"Latency: P50={metrics['predictions']['latency_p50']:.0f}ms, "
        ...
        logger.info(
            f"Games: {metrics['games']['created']} created, "
            f"{metrics['games']['active']} active, "
            f"{metrics['games']['completed']} completed"
        )
        logger.info(
            f"Retraining: {metrics['retraining']['triggered']} triggered, "
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if latency_ms > 500:
                    logger.warning(
                        f"Slow request: {metric_name} took {latency_ms:.0f}ms"
                    )
