...
export default function QuickDrawApp() {
  ...
  const handleRoundComplete = async (imageData, success, winner = null, timeLeft = 0, confidence = 0) => {
    ...
    // In multiplayer mode RACE: any player who wins advances the round
    // In TEAM mode: only host handles transitions
    if (gameMode === 'RACE' && roomCode && success) {
      // In RACE mode, the winner triggers the round transition
      setTimeout(async () => {
        try {
      ...
      }, 2000); // Wait 2 seconds for animations
    } else if (gameMode === 'TEAM' && roomCode && isHost) {
      // In TEAM mode, only host advances rounds
      setTimeout(async () => {
        try {
          if (round < totalRounds) {
            await MultiplayerService.nextRound(roomCode, wordsToDrawFr);
